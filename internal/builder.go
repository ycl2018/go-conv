...
func NewBuilder(f *ast.File, types *types.Package, importer *Importer) *Builder {
	return &Builder{
		...
		types:       types,
		importer:    importer,
		genFuncName: make(map[string]struct{}),
	}
}

func (b *Builder) BuildFunc(dst, src types.Type) {
...
func (b *Builder) Generate() ([]byte, error) {
	...
	err := printer.Fprint(&sb, token.NewFileSet(), b.f)
	if err != nil {
		return nil, fmt.Errorf("[go-conv]: failed to generate code: %w", err)
	}
	return sb.Bytes(), nil
}
