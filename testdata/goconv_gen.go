...
func CopyDomain2CategoryToDomainCategory(src *domain2.Category) (dst *domain.Category) {
	if src != nil {
		dst = new(domain.Category)
		dst.CategoryID = src.CategoryID
		dst.Name = src.Name
		dst.Foo = CopyDomain2FooToDomainFoo(src.Foo)
	}
	return
}
func CopyDomain2FooToDomainFoo(src *domain2.Foo) (dst *domain.Foo) {
	if src != nil {
		dst = new(domain.Foo)
		if src.Bar != nil {
			dst.Bar = new(string)
...
func CopyModelPetToDomainEmbed(src *model.Pet) (dst *domain.Embed) {
	if src != nil {
...
func CopyModelPetToDomainPet(src *model.Pet) (dst *domain.Pet) {
	if src != nil {
		dst = new(domain.Pet)
		dst.ID = src.ID
		dst.Name = new(string)
		*dst.Name = src.Name
		...
		for i := 0; i < 3 && i < len(src.Array); i++ {
			dst.Array[i] = CopyDomain2CategoryToDomainCategory(src.Array[i])
		}
		if len(src.Slices) > 0 {
			dst.Slices = make([]*domain.Category, len(src.Slices))
			for i := 0; i < len(src.Slices); i++ {
				dst.Slices[i] = CopyDomain2CategoryToDomainCategory(src.Slices[i])
			}
		}
		if len(src.Maps) > 0 {
		...
		dst.A = src.A
		dst.B = src.B
		dst.Embed = CopyModelPetToDomainEmbed(src)
	}
	return
}
...
func init() {
	ModelToDomain2 = CopyModelPetToDomainPet
